(() => {
  if (!document.body.classList.contains('page-home-unified')) return;

  const THRESHOLD = 0.88;
  const screens = () => [
    ...document.querySelectorAll('.home-hero-stack, .section-screen[id]'),
  ];
  ...
  document.querySelectorAll('.nav-anchor a[href^="#"], .hero a[href^="#"], a.hero-handyman[href^="#"]').forEach((link) => {
  ...
  window.addEventListener('home-scroll-lock', (event) => {
